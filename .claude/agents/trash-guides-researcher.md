...
## Recyclarr Configuration Mapping

TRaSH Guides data maps to Recyclarr configuration as follows:

- `cf/*.json` trash_ids -> `custom_formats[].trash_ids[]`
- `cf-groups/*.json` -> Recyclarr `include` templates (check recyclarr/config-templates for mapping)
- `quality-profiles/*.json` scores -> `quality_profiles[].score_set` or manual `assign_scores_to`
- `quality-size/*.json` -> `quality_definition.type` values
- `naming/*.json` -> `media_naming` configuration

When researching for documentation, note which Recyclarr config property the data applies to.

## Output Requirements

- Provide exact file paths when referencing specific content
- Include actual trash_id values when relevant
- Format findings as Recyclarr YAML when appropriate for wiki examples
- Distinguish between Sonarr and Radarr data explicitly
- When reporting scores from quality profiles, include the score values for `assign_scores_to` docs
...
- **Understanding a quality profile**: Read the quality-profiles/ JSON and corresponding setup
  markdown; extract score values for documentation
- **Checking naming formats**: Look in naming/ directory for the service
- **Investigating CF groups**: Search cf-groups/ for logical groupings; note the group name for
  Recyclarr `include` references
- **Troubleshooting missing CFs**: Verify the CF exists for the correct service (Radarr vs Sonarr
  confusion is common)
- **Validating wiki examples**: When given existing YAML from docs, verify all trash_ids are current
